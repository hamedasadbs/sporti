...
export const InfoTables = (props) => {
  ...
  useEffect(() => {
    axios({
      method: "get",
      url: `http://10.42.0.72:44351/api/Bus/LastRequests?PageSize=5&PageNumber=${lastRequestsPage}`,
    }).then((res) => {
  ...
  useEffect(() => {
    axios({
      method: "get",
      url: `http://10.42.0.72:44351/api/Bus/LastResponses?PageSize=5&PageNumber=${lastResponsesPage}`,
    }).then((res) => {
  ...
  useEffect(() => {
    axios({
      method: "get",
      url: `http://10.42.0.72:44351/api/Bus/MostUsedSoftware?PageSize=5&PageNumber=${mostUsedSoftwarePage}`,
    }).then((res) => {
  ...
  useEffect(() => {
    axios({
      method: "get",
      url: `http://10.42.0.72:44351/api/Software/Responses?software=${props.software}&PageSize=5&PageNumber=${responsesPage}`,
    }).then((res) => {
  ...
  useEffect(() => {
    axios({
      method: "get",
      url: `http://10.42.0.72:44351/api/Software/Requests?software=${props.software}&PageSize=5&PageNumber=${requestsPage}`,
    }).then((res) => {
  ...
  useEffect(() => {
    axios({
      method: "get",
      url: `http://10.42.0.72:44351/api/Software/Transactions?software=${props.software}&PageSize=5&PageNumber=${transactionsPage}`,
    }).then((res) => {
  ...
  const infoTable = {
    bus: [
      ...
      },
      {
        ...
        records: mostUsedSoftware.map((res) => [
          res.total,
          res.responseCount,
          res.requestCount,
          res.softwareName,
  ...
  return (
    <article className={style.infoTables}>
      {props.infoIndex == "bus"
        ...
        : infoTable.softwares.map((it, index) => (
            <InfoTable
              title={it.title}
              toples={it.toples}
              records={it.records}
